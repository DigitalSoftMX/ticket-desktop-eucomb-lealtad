                ...
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        for(i=0;i<response.pointsadded.length;i++){
                            $("#datatable_1").find('tbody').append(
                                '<tr><td>'+response.pointsadded[i].sale+'</td><td>'+response.pointsadded[i].station+'</td><td>'+response.pointsadded[i].liters+'</td><td>'+response.pointsadded[i].points+'</td><td>'+response.pointsadded[i].concepto+'</td><td>'+response.pointsadded[i].date+'</td></tr>'
                            );
                        }
                        for(i=0;i<response.pointsubstracted.length;i++){
                            $("#datatable_2").find('tbody').append(
                                '<tr><td>'+response.pointsubstracted[i].exchange+'</td><td>'+response.pointsubstracted[i].station+'</td><td>'+response.pointsubstracted[i].status+'</td><td>'+response.pointsubstracted[i].points+'</td><td>'+response.pointsubstracted[i].type+'</td><td>'+response.pointsubstracted[i].date+'</td></tr>'
                            );
                        }
                        iniciar_date('datatable_1');
                        iniciar_date('datatable_2');
